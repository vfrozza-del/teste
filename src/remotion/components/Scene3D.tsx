...
const AnimatedText3D: React.FC<{ text: string; color: string; position?: [number, number, number] }> = ({
  ...
  useFrame(() => {
    if (groupRef.current) {
      groupRef.current.rotation.y = interpolate(frame, [0, durationInFrames], [0, Math.PI * 0.3]);
    }
  });
...
export const Scene3D: React.FC<{ config: Scene3DConfig }> = ({ config }) => {
  ...
  const cameraZ = cameraAnimation === 'orbit' ? Math.cos(orbitAngle) * 5 :
                  cameraAnimation === 'zoom-in' ? interpolate(frame, [0, durationInFrames], [8, 4]) :
                  cameraAnimation === 'zoom-out' ? interpolate(frame, [0, durationInFrames], [4, 8]) : 5;
